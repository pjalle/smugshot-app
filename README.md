...
- `windows/` is also the Linux version. `main_linux.go` picks the half to run; `x11_linux.go` is the X11 one (the shortcut, the drag layer, the picture, the window under the drag, the clipboard); `wayland_linux.go` is the Wayland one (the drag layer as a layer-shell or fullscreen surface, the shm buffers, the pointer, the clipboard through wl_data_device); `capture_linux.go` takes the Wayland picture, by the wlroots screencopy protocol or the desktop's screenshot service. `wlproto/` holds the two wlroots protocols, generated from their XML by the wayland-scanner (`go generate ./wlproto/...`); do not edit them by hand. `store.go`, `render.go`, `settings.go` and `sound.go` are shared with Windows. `./scripts/build-linux.sh` builds it.
...
**Settings…** in the tray menu opens a small window (`windows/settings_window.go`): the shortcut (`ctrl+shift+1`, `ctrl+alt+s`, `win+shift+f9`: modifiers plus one key, typed as text), the folder with a Choose… button, how long to keep (the same choices as on the Mac), the clipboard text, the sound (Feed discovered, Menu command, Navigation start and Speech on, which ship with Windows, or Tink, or Off) and the "Copied" banner. Every change is saved as it is made, and a new shortcut works at once. Behind the window is a small file, `%APPDATA%\Smugshot\settings.json`, which can also be edited by hand (`folder`, `keepFor`, `prefix`, `hotkey`, `sound` by name or `off`, `banner`); a hotkey changed by hand needs a restart. The tray menu starts with the shortcut to press and also has "Copy last smugshot again", the last five smugshots, the Sound choices and the Banner switch, and "Check for updates…", which opens smugshot.io in your browser.
...
It works on both kinds of Linux session, but not in the same way, because Wayland and X11 are not the same kind of thing.

**On X11** it behaves like the Windows version: start it once (`./smugshot &`), it stays running and holds **Ctrl + Shift + 1** for the whole desktop. shot.md names the app and the window under the drag.

**On Wayland** no program may hold a shortcut for the whole desktop, so one run is one smugshot: bind `smugshot` to a key in your desktop's own keyboard settings (`smugshot --help` prints the steps for GNOME, KDE, sway and Hyprland). Press your key, drag, paste. Wayland also forbids asking what is under the pointer, so shot.md has no app or window line there and says so. The picture comes from the compositor: straight from it on sway, Hyprland and other wlroots desktops, and through the desktop's own screenshot service on GNOME and KDE, which asks once whether Smugshot may do it and remembers the answer. Smugshot stays running for ten minutes after a smugshot, because on Wayland the clipboard is served by whichever program put it there.

Settings are the same file as on Windows, `~/.config/Smugshot/settings.json`, written on first start: the folder, `keepFor`, the clipboard text, the `hotkey` (X11 only: `ctrl+shift+1`, `ctrl+alt+s`, `win+shift+f9`, where win is the Super key; needs a restart), `sound` (anything but `off` plays the tink) and `banner` (the notification after a smugshot). No tray icon, no Settings window and no "what was there" yet.

Tried so far, in a VM only (an Ubuntu machine in OrbStack, 2026-09-20): X11 in a virtual screen with Xvfb and Openbox, and Wayland in a headless sway session driven by a virtual mouse. In both, the gesture, the files, the clipboard and every way to cancel work. Nobody has run it on a real Linux desktop yet, and GNOME and KDE have never been tried at all: the page says so and asks for word back. The download is `smugshot.io/download/linux`, a tar.gz with both files, made by `./scripts/build-linux.sh <version>` and put on the page with each release like the Mac and Windows files.
