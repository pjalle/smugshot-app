...
// Smugshot on Linux. Which half runs depends on the session: X11 (x11_linux.go) or Wayland
// (wayland_linux.go). They work differently because the two do; the README says how.
package main
...
// runX11 is the X11 version: a program that stays running, holds the shortcut for the whole desktop, and
// takes smugshots when it is pressed.
func runX11() {
	var err error
